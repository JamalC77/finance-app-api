...
export async function sendPasswordResetEmail(email: string, resetLink: string, username: string): Promise<boolean> {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: env.EMAIL_FROM || "CFO Line <noreply@thecfoline.com>",
      to: email,
      subject: "Reset Your Password - CFO Line",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Reset Your Password</h2>
          <p>Hello ${username},</p>
          <p>We received a request to reset your password for your CFO Line account.</p>
          <p>To reset your password, please click the button below. This link is valid for 30 minutes.</p>
          <div style="text-align: center; margin: 30px 0;">
            <a href="${resetLink}" style="background-color: #4a86e8; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: bold;">Reset Password</a>
          </div>
          <p>If you did not request a password reset, please ignore this email or contact support if you have concerns.</p>
          <p>Thanks,<br>The CFO Line Team</p>
          ...
          <div style="background-color: #f8f9fa; border: 1px solid #e9ecef; border-radius: 5px; padding: 15px; margin: 20px 0;">
            ...
            <table style="width: 100%; border-collapse: collapse;">
              <tr>
                ...
                <td style="padding: 8px 0; border-bottom: 1px solid #dee2e6; font-weight: bold; text-align: right;">${formatCurrency(
                  payment.amount
                )}</td>
              </tr>
              <tr>
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 20px;">
          ...
          <p>Good news! ${contactName} has made a payment of <strong>${formatCurrency(payment.amount)}</strong> for invoice #${
